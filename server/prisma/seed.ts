...
function j(arr: unknown[]) {
  return JSON.stringify(arr);
}
function jObj(arr: { title: string; content: string }[]) {
  return JSON.stringify(arr);
}
function jFaq(arr: { q: string; a: string }[]) {
  return JSON.stringify(arr);
}
function jTestimonials(arr: { text: string; author: string; role?: string }[]) {
  return JSON.stringify(arr);
}
function jInstructors(arr: { name: string; role: string; bio: string }[]) {
  return JSON.stringify(arr);
}
...
async function main() {
  ...
  const p1 = await prisma.program.create({
    data: {
      ...
      howItWorks: "Каждую неделю вы смотрите видеоуроки в удобное время, выполняете практические задания и получаете обратную связь от куратора. Раз в неделю — общий созвон с разбором кейсов и ответами на вопросы. К концу курса вы запускаете свою первую кампанию и защищаете дипломный проект.",
      testimonials: jTestimonials([
      ...
      instructors: jInstructors([
        { name: "Максим Волков", role: "Ведущий преподаватель", bio: "12 лет в performance-маркетинге. Запускал рекламу для e-commerce и образовательных проектов. Практикующий таргетолог." },
      ]),
      orderNum: 0,
    },
  });
  ...
  const p2 = await prisma.program.create({
    data: {
      ...
      howItWorks: "Обучение идёт в формате коротких видеоуроков и еженедельных воркшопов в Zoom. После каждого модуля — задание с проверкой. В чате можно задавать вопросы и обмениваться опытом с другими участниками.",
      testimonials: jTestimonials([
      ...
      instructors: jInstructors([
        { name: "Юлия Белова", role: "Автор курса", bio: "Копирайтер и контент-стратег. 8 лет в digital. Ведёт соцсети брендов и обучает контент-маркетингу." },
      ]),
      orderNum: 1,
    },
  });
  ...
  const p3 = await prisma.program.create({
    data: {
      ...
      howItWorks: "Уроки в записи доступны в личном кабинете. Каждую неделю — практика в выбранном сервисе рассылок и разбор кейсов на созвоне. Итоговая работа — готовая воронка с триггерными письмами.",
      testimonials: jTestimonials([
        { text: "За месяц настроил welcome-цепочку и автоматизацию после покупки. Конверсия в повторные заказы выросла. Преподаватель помог с сегментацией.", author: "Алексей П.", role: "интернет-магазин" },
      ...
      instructors: jInstructors([
        { name: "Андрей Соколов", role: "Эксперт по email-маркетингу", bio: "10+ лет в маркетинге. Внедрял воронки для образовательных платформ и e-commerce." },
      ]),
      level: "Начальный",
